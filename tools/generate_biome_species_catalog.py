...
# Unlike the broad type-derived biomes, Underwater is intentionally curated.
# It combines aquatic species with ice-cavern, dragon, fossil, ruin, abyssal,
# and freshwater species to make Dive encounters feel like a distinct biome.
UNDERWATER_SPECIES = frozenset(
    """
    MAGIKARP GYARADOS HORSEA SEADRA KINGDRA TENTACOOL TENTACRUEL SHELLDER
    CLOYSTER STARYU STARMIE CHINCHOU LANTURN CORSOLA REMORAID OCTILLERY
    WAILMER WAILORD RELICANTH CLAMPERL HUNTAIL GOREBYSS LUVDISC FINNEON
    LUMINEON FRILLISH JELLICENT ALOMOMOLA SKRELP DRAGALGE CLAUNCHER
    CLAWITZER MAREANIE TOXAPEX WISHIWASHI_SOLO PYUKUMUKU BRUXISH DHELMISE
    ARROKUDA BARRASKEWDA CRAMORANT CHEWTLE DREDNAW FINIZEN PALAFIN_ZERO VELUZA
    DONDOZO TATSUGIRI_CURLY LAPRAS MANTYKE MANTINE WIMPOD GOLISOPOD BINACLE
    BARBARACLE INKAY MALAMAR DEWPIDER ARAQUANID CRABRAWLER CRABOMINABLE
    CORPHISH CRAWDAUNT CARVANHA SHARPEDO QWILFISH OVERQWIL DREEPY DRAKLOAK
    DRAGAPULT EISCUE_ICE STUNFISK GRAPPLOCT ORTHWORM SHUCKLE OMANYTE OMASTAR
    KABUTO KABUTOPS ARCHEN
    GOLDEEN SEAKING KRABBY KINGLER CLOBBOPUS PINCURCHIN WIGLETT WUGTRIO
    SANDYGAST PALOSSAND TIRTOUGA CARRACOSTA LILEEP CRADILY ANORITH ARMALDO
    CURSOLA STONJOURNER MIMIKYU_DISGUISED MINIOR SPHEAL SEALEO WALREIN SEEL DEWGONG
    SNOVER ABOMASNOW SNORUNT GLALIE FROSLASS BERGMITE AVALUGG CUBCHOO
    BEARTIC SNOM FROSMOTH CETODDLE CETITAN DELIBIRD VANILLITE DRATINI
    DRAGONAIR DRAGONITE BAGON SHELGON SALAMENCE GIBLE GABITE GARCHOMP DEINO
    ZWEILOUS HYDREIGON GOOMY SLIGGOO GOODRA JANGMO_O HAKAMO_O KOMMO_O
    NOIBAT ARCHEOPS AERODACTYL CRANIDOS RAMPARDOS SHIELDON BASTIODON TYRUNT
    TYRANTRUM AMAURA AURORUS DRACOZOLT ARCTOZOLT DRACOVISH ARCTOVISH GOLETT
    GOLURK BALTOY CLAYDOL YAMASK COFAGRIGUS SIGILYPH ZUBAT GOLBAT CROBAT
    WOOBAT SWOOBAT SABLEYE SPIRITOMB MISDREAVUS MISMAGIUS LITWICK LAMPENT
    CHANDELURE GASTLY HAUNTER GENGAR DRIFLOON DRIFBLIM ROTOM_HEAT KLEFKI BRONZONG
    POLIWAG POLIWHIRL POLIWRATH POLITOED LOTAD LOMBRE LUDICOLO WOOPER
    QUAGSIRE MUDKIP MARSHTOMP SWAMPERT BARBOACH WHISCASH FEEBAS MILOTIC
    TYMPOLE PALPITOAD SEISMITOAD BASCULEGION_M
    """.split()
)
...
def species_biomes(species: Species) -> tuple[str, ...]:
    ...
    if species.constant in UNDERWATER_SPECIES:
        biomes.add("Underwater")
