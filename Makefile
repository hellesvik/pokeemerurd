...
RELEASE      ?= 0
# Builds a playtesting ROM with selected navigation and battle helpers.
TEST_MODE    ?= 0
...
ifeq ($(TEST_MODE),1)
	FILE_NAME := pokeemerurd-testmode
endif
...
OBJ_DIR_NAME_TESTMODE := build/emerald-testmode
...
CPPFLAGS := $(INCLUDE_CPP_ARGS) -Wno-trigraphs -DMODERN=1 -DTESTING=$(TEST) -DTEST_MODE=$(TEST_MODE) -D$(GAME_VERSION) -std=gnu17
ifeq ($(RELEASE),1)
...
.PHONY: all rom agbcc modern compare check debug release testmode
.PHONY: $(RULES_NO_SCAN)
